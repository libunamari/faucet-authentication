...
class Faucet(ABCRyuApp):
    ...
    _APP_NAME = "Faucet_Controller"
    _EXPECTED_HANDLERS = (ofp_event.EventOFPPacketIn.__name__,
                          ofp_event.EventOFPSwitchFeatures.__name__,
                          ofp_event.EventOFPPortStatus.__name__,
                          ofp_event.EventOFPErrorMsg.__name__,
                          EventFaucetReconfigure.__name__,
    ...
    def reload_config(self, ryu_event):
        ...
        for new_dp in new_dps:
            ...
            ryudp = self._contr.get(new_dp.dp_id)
    ...
    def packet_in(self, ryu_event):
        ...
        if ryu_event.msg.table_id < self._faucet_table_id:
            print "Faucet not dealing with packet in messages from other tables"
            return
    ...
    def switch_features(self, ryu_event):
    ...
    def _verify_contr_handlers(self):
        ...
        if not len(failures) == 0:
            print str(self._APP_NAME) + "The following OpenFlow protocol events are not supported by the controller:"
            for f in failures:
                print "\t- " + str(f)
            return False
        else:
    ...
    def _bgp_route_handler(self, path_change, vlan):
        ...
        ryudp = self._contr.get(valve.dp.dp_id)
    ...
    def gateway_resolve_request(self):
        ...
        while True:
            #print "Faucet: gateway resolve request"
            self._contr.send_event('dpset', EventFaucetResolveGateways())
            hub.sleep(2)

    def host_expire_request(self):
        """Trigger expiration of host state in controller."""
        while True:
            #print "Faucet: host expire request"
            self._contr.send_event('dpset', EventFaucetHostExpire())
            hub.sleep(5)
